...
class Cache:
    ...
    def _check_cache_integrity(self, cache_key):
        ...
        for key, val in outputs.dict(exclude_unset=True).items():
            if self.output_spec.__fields__[key].type_ == FilePath:
                assert os.path.exists(val), f"Broken cache {cache_key}, missing file: {val}"
    ...
    def _record_cache_access(self, cache_key):
        with open(os.path.join(self.cache_directory,cache_key, CACHE_LAST_ACCESSED_FNAME), "w") as f:
            f.write(str(time.time()))

    def _get_cache_last_accessed(self, cache_key):
        with open(os.path.join(self.cache_directory,cache_key, CACHE_LAST_ACCESSED_FNAME), "r") as f:
            return float(f.read())
    ...
    def _save_to_cache(self,cache_key,outputs:BaseModel, directory):
        ...
        if not os.path.exists(cache_dir):
            ...
            outputs_cache = self._change_root_response(outputs, directory, cache_dir_files)
